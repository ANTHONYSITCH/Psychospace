...
import { getChatHistory, loadProfile, sendChatMessage, transcribeAudio } from '../api'
...
export default function Companion() {
  ...
  const [audioRecorder] = useState(() => createAudioRecorder(window, transcribeAudio))
  const [audio, setAudio] = useState(() => audioRecorder.getSnapshot())
  ...
  const recording = microphone.listening || microphone.starting || audio.listening || audio.starting
  const transcribing = audio.transcribing
  const listening = microphone.listening || audio.listening
  ...
  useEffect(() => {
    const unsubscribe = audioRecorder.subscribe(setAudio)
    return () => { unsubscribe(); audioRecorder.abort() }
  }, [audioRecorder])
  function startListening() {
    voice.stop(); setDictationNotice('')
    const service = microphone.availability === 'available' ? recognition : audioRecorder
    service.start(text => {
      if (!mounted.current) return
      setDraft(previous => previous.trim() ? `${previous.trim()} ${text}` : text)
      setDictationNotice('Voix transcrite sur cet appareil. Tu peux relire et corriger ton message avant de l’envoyer.')
    })
  }
  ...
  function voiceControl(item) {
    ...
    return <button className="voice-listen" type="button" disabled={!voiceEnabled || !speech.supported || !speech.selectedVoice || sending || recording || transcribing} aria-label={active ? 'Arrêter la lecture de cette réponse' : 'Écouter cette réponse'} onClick={() => active ? voice.stop() : voice.speak(item.content, speechKey(item))}>{active ? 'Arrêter' : 'Écouter'}</button>
  }
  ...
  async function send(event) {
    ...
    if (audioRecorder.getSnapshot().starting || audioRecorder.getSnapshot().listening || audioRecorder.getSnapshot().transcribing) return
  ...
  return <>
    ...
    <section className="conversation-space" aria-label="Échanger avec PsychoSpace">
      <div className="conversation-presence"><PsychoSpacePresence state={sending ? 'thinking' : transcribing ? 'transcribing' : listening ? 'listening' : speech.speaking ? 'speaking' : 'attentive'} />
        <p className="conversation-status" role="status" aria-live="polite">{sending ? late ? 'Le compagnon local se prépare… Je prends le temps de te répondre.' : 'PsychoSpace réfléchit…' : transcribing ? 'Je transforme ta voix en texte...' : listening ? 'Je t’écoute...' : speech.speaking ? 'PsychoSpace te répond à voix haute.' : phase === 'loading' ? 'Je retrouve notre conversation…' : 'Je suis là.'}</p>
      </div>
      ...
      <form className="conversation-composer" onSubmit={send}>
        ...
        <div className="microphone-controls"><button type="button" className="microphone-button" aria-label={recording ? 'Arrêter le microphone' : 'Parler à PsychoSpace'} aria-pressed={recording} disabled={!canCompose || transcribing || (!recording && microphone.availability !== 'available' && !audio.supported)} onClick={() => recording ? (recognition.stop(), audioRecorder.stop()) : startListening()}><svg viewBox="0 0 24 24" width="17" height="17" fill="none" stroke="currentColor" strokeWidth="1.5" aria-hidden="true"><rect x="9" y="3" width="6" height="12" rx="3" /><path d="M6 11v1a6 6 0 0 0 12 0v-1M12 18v4M8 22h8" /></svg>{recording ? 'Arrêter' : 'Parler'}</button><span aria-live="polite">{(microphone.starting || audio.starting) ? 'Ouverture du microphone…' : microphone.checking && !recording ? 'Vérification de la reconnaissance locale…' : microphone.installing || microphone.availability === 'downloading' ? 'Installation en cours...' : microphone.availability === 'available' ? 'Reconnaissance vocale prête.' : ''}</span></div>
        ...
        {(audio.error || microphone.error) && <p className="microphone-notice" aria-live="polite">{audio.error || microphone.error}</p>}
        {!audio.supported && microphone.availability !== 'available' && <p className="microphone-notice">La transcription vocale locale n'est pas disponible pour le moment. Tu peux continuer à m'écrire.</p>}
        <p className="microphone-notice">L'audio est traité temporairement par PsychoSpace sur cet appareil et supprimé après transcription.</p>
        ...
        <div className="composer-actions"><span>Entrée pour envoyer · Maj + Entrée pour une nouvelle ligne</span><button className="primary-button" disabled={!canCompose || !draft.trim() || recording || transcribing} type="submit">{sending ? 'Je prends un instant…' : phase === 'send-error' ? 'Réessayer' : 'Envoyer'}{!sending && <Icon name="arrow" />}</button></div>
      </form>
